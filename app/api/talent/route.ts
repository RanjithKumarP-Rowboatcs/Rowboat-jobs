...
async function getPoolCandidates(supabase: any) {
  ...
  const candidates = (data || []).map((c: any) => ({
    candidate_id: c.id,
    talent: { technology: c.technology, primary_skill: c.primary_skill, secondary_skills: c.secondary_skills || [], industry: c.industry, previous_companies: c.previous_companies || [], employment_type: c.employment_type, work_authorization: c.work_authorization, certifications: c.certifications || [], projects: c.projects || [], availability: c.availability, ai_generated_skill_profile: c.ai_summary, candidate_status: c.candidate_status, source: c.source, recruiter: c.recruiter, recruiter_notes: c.recruiter_notes },
    profile: { id: c.id, full_name: c.full_name, email: c.email, phone: c.phone, location: c.location, current_company: c.current_company, experience_years: c.years_experience, notice_period_days: c.notice_period_days, resume_url: c.resume_storage_path ? `storage:${c.resume_storage_path}` : null, linkedin_url: c.linkedin_url, current_ctc: c.current_compensation, expected_ctc: c.expected_compensation },
  }))
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    // Preview mode extracts fields but deliberately does not save anything.
    ...
    const candidate = {
      ...
      technology: formText(form, 'technology', parsed.technology), primary_skill: primarySkill, secondary_skills: skills,
      years_experience: formNumber(form, 'years_experience', parsed.experienceYears), industry: formText(form, 'industry', parsed.industry), current_company: formText(form, 'current_company', parsed.currentCompany),
      previous_companies: formArray(form, 'previous_companies', parsed.previousCompanies), notice_period_days: formNumber(form, 'notice_period_days', parsed.noticePeriodDays),
      availability: formText(form, 'availability', parsed.noticePeriodDays === 0 ? 'Immediate' : parsed.noticePeriodDays != null ? `${parsed.noticePeriodDays} days` : null),
      ...
      employment_type: formText(form, 'employment_type', null), work_authorization: formText(form, 'work_authorization', null), certifications: formArray(form, 'certifications', parsed.certifications),
      ...
      resume_text: parsed.text, ai_summary: parsed.aiSummary, ai_extracted_skills: [primarySkill, ...skills].filter(Boolean), ai_certifications: parsed.certifications, parsed_data: parsed,
      parse_status: 'completed', parse_error: null, candidate_status: asText(form.get('candidate_status')) || 'active', source: asText(form.get('source')) || 'Resume upload',
      recruiter: asText(form.get('recruiter')) || null, recruiter_notes: asText(form.get('recruiter_notes')) || null,
    }
...
export async function PATCH(request: NextRequest) {
  ...
  if (!user) return NextResponse.json({ error: 'Sign in required' }, { status: 401 })
  if (!staffOnly(role)) return NextResponse.json({ error: 'Recruiter/admin access required' }, { status: 403 })
  const body = await request.json(); const candidateId = asText(body.candidate_id)
...
export async function POST(request: NextRequest) {
  ...
  if (!staffOnly(role)) return NextResponse.json({ error: 'Recruiter/admin access required' }, { status: 403 })
  const body = await request.json()
  const requirement = { text: asText(body.text), location: asText(body.location), skills: Array.isArray(body.skills) ? body.skills.map(asText).filter(Boolean) : asText(body.skills).split(',').map(s => s.trim()).filter(Boolean), experienceMin: Number(body.experienceMin || 0), noticeDays: body.noticeDays === undefined || body.noticeDays === '' ? undefined : Number(body.noticeDays) }
  ...
  if (profileError) return NextResponse.json({ error: profileError.message }, { status: 500 })
  const profileMap = new Map((profiles || []).map((p: any) => [p.id, p]))
  const matches = (talent || []).map((t: any) => { const profile = profileMap.get(t.candidate_id) || null; return { candidate_id: t.candidate_id, talent: t, profile, score: candidateScore({ talent: t, profile }, requirement) } }).sort((a: any, b: any) => b.score - a.score).slice(0, 25)
