...
function normalizedPhone(value: unknown) { return text(value).replace(/\D/g, '') }
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    const email = normalizedEmail(form.get('email') || parsed.email)
    const phone = normalizedPhone(form.get('phone') || parsed.phone)
    let existing: any = null
    if (email) {
      const result = await supabase.from('talent_pool_candidates').select('id,full_name,email').eq('email_normalized', email).limit(1).maybeSingle()
      if (result.error) throw result.error
      existing = result.data
    }
    if (!existing && phone.length >= 10) {
      const result = await supabase.from('talent_pool_candidates').select('id,full_name,email').eq('phone_normalized', phone).limit(1).maybeSingle()
      if (result.error) throw result.error
      existing = result.data
    }
    if (existing) return NextResponse.json({ error: 'This candidate already exists in the Talent Database. Use Edit or Re-read resume on the existing profile instead of adding a duplicate.', existing_candidate_id: existing.id, existing_name: existing.full_name }, { status: 409 })
