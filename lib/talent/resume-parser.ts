...
function extractPhone(text: string) {
  ...
  return matches.map(v => v.trim()).find(v => v.replace(/\D/g, '').length >= 10 && v.replace(/\D/g, '').length <= 15)
}
...
function extractName(text: string) {
  ...
  const bad = /resume|curriculum|vitae|profile|summary|objective|contact|email|phone|linkedin|github|skills|experience/i
...
function extractCertifications(text: string) {
  const section = firstMatch(text, [/(?:certifications?|certificates?)\s*[:\-]?\s*([\s\S]{0,1600})/i]) || ''
  return section.split(/\n|•|,|;/).map(x => x.trim()).filter(x => x.length > 3 && x.length < 120).slice(0, 15)
}

function extractProjects(text: string) {
  const section = firstMatch(text, [/(?:projects?|key projects?|project experience)\s*[:\-]?\s*([\s\S]{0,2200})/i]) || ''
  return section.split(/\n|•/).map(x => x.trim()).filter(x => x.length > 10 && x.length < 240).slice(0, 15)
}

function extractEducation(text: string) {
  const section = firstMatch(text, [/(?:education|academic background|qualifications?)\s*[:\-]?\s*([\s\S]{0,1600})/i]) || ''
...
function inferIndustry(text: string, skills: string[]) {
  ...
  const rules: Array<[string, string[]]> = [
    ['Information Technology', ['software','technology','it services','saas','cloud','developer','engineering']],
...
function makeSummary(name: string | undefined, years: number | undefined, skills: string[], company: string | undefined, location: string | undefined, certifications: string[]) {
  ...
  const skillText = skills.slice(0, 8).join(', ') || 'relevant skills'
