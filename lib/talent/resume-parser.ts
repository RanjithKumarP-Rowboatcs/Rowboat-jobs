...
function extractDateOfBirth(text: string) { const m=text.match(/(?:date of birth|dob|birth date)\s*[:\-]?\s*(\d{1,2}[\/-]\d{1,2}[\/-](?:19|20)\d{2}|(?:\d{1,2}\s+)?[A-Za-z]{3,9}\s+\d{1,2},?\s+(?:19|20)\d{2})/i); return m?.[1] }
