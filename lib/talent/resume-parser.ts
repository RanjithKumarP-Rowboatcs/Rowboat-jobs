...
function isHeading(line: string) { const value = line.toLowerCase().replace(/^[-•●➢▪◦*\s]+/, '').replace(/[:\-]+$/, '').trim(); return SECTION_NAMES.some(name => value === name || value.startsWith(`${name} `)) }
...
function extractSectionFlexible(text:string,headings:string[]){const lines=linesOf(text);const aliases=headings.map(h=>h.toLowerCase());const start=lines.findIndex(line=>aliases.includes(line.toLowerCase().replace(/^[-•●➢▪◦*\s]+/,'').replace(/[:\-]+$/,'').trim()));if(start<0)return[];const values:string[]=[];for(let i=start+1;i<lines.length&&values.length<60;i++){if(isHeading(lines[i]))break;const value=lines[i].replace(/^[\-–—•●➢▪◦*]\s*/,'').trim();if(value.length>=3&&value.length<=800)values.push(value)}return values}
