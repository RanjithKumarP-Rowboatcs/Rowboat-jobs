...
function extractCurrentEmployment(text: string) {
  ...
  const lines=linesOf(text)
  const startIndex=lines.findIndex(line=>/^(professional\s+|work\s+|employment\s+|career\s+)?(experience|history)$/i.test(line))
  const window=lines.slice(startIndex>=0?startIndex+1:0,startIndex>=0?startIndex+70:70)
  for(let i=0;i<window.length;i++){
    const line=window[i]
    const next=window[i+1]||''
    const combined=line+' '+next
    const hasCurrent=/\b(?:present|current|till\s+date|ongoing)\b/i.test(combined)
    const direct=line.match(/^(.+?)\s*[-–—|:]\s*([A-Za-z][A-Za-z0-9 /&.'()+-]{2,100})$/)
    if(direct&&hasCurrent)return{company:direct[1].trim(),role:direct[2].trim()}
    const withLocation=line.match(/^(.+?)\s*[-–—|:]\s*([A-Za-z][A-Za-z0-9 /&.'()+-]{2,100})$/)
    if(withLocation&&/\b(?:Senior|Lead|Principal|Consultant|Engineer|Developer|Architect|Analyst|Manager|Specialist)\b/i.test(withLocation[2])&&hasCurrent)
      return{company:withLocation[1].trim(),role:withLocation[2].trim()}
    const parenthesized=line.match(/^(.+?)\s*\(([^)]*(?:current|present|till date)[^)]*)\)\s*[-–—|:]\s*(.+)$/i)
    if(parenthesized)return{company:parenthesized[1].trim(),role:parenthesized[3].trim()}
  }
